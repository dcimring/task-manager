...
*   **Focus Page Layout**: The Focus page uses a clean, transparent column grid with no backgrounds, borders, or shadows. Each column header displays the category name (e.g., "Overdue", "Blocked") and task count aligned baseline, with a solid colored accent bar directly underneath indicating status urgency. Task cards are clean white surfaces styled with generous padding, using serif typography for readability (task title) and system/mono fonts for metadata (project, date, priority). Cards in Blocked and Suggested columns feature status capsules mapped directly to status tokens from `design.md` (e.g. Blocked, In progress).
